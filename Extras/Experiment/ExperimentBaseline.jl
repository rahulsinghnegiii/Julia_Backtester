...
function get_rsi(
    ...
    cache_key = "rsi_$(period_str)_1500_$(end_date)"
    return get_cached_value(
        cache, cache_key, () -> calculate_rsi(ticker, length_data, period, end_date)
    )
end

function run_single_experiment(tickers, length_data, period, end_date, cache)
    for _ in 1:5
        for i in 1:20
            get_rsi(tickers[i], length_data, period, end_date, cache)
        end
    end
end
...
function run_experiments(n_experiments)
    ...
    for _ in 1:n_experiments
        @time run_single_experiment(tickers, 250, 1500, Date("2024-05-31"), cache)
    end
end
