...
function get_historical_stock_data_arrow(
    ...
    try
        ...
        file_path = joinpath(tempdir, "$ticker.arrow")
...
# mid level function (historical data, checks for arrow)
function get_historical_stock_data(ticker::String, period::Int, end_date::Date, tempdir)
    try
        ...
        if isfile(file_path)
            historical_data = get_historical_stock_data_arrow(
...
function setup_temp_dir(tickers::Vector{String})
    ...
    for ticker in tickers
        local_arrow_path = joinpath("./ArrowData", "$ticker.arrow")
        temp_arrow_path = joinpath(tempdir, "$ticker.arrow")
        cp(local_arrow_path, temp_arrow_path)
    end
